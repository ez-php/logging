...
- **`boot()` re-binds the core `ExceptionHandlerInterface` — a deliberate decorator exception** — `LogServiceProvider::boot()` resolves the already-bound handler and re-binds `ExceptionHandlerInterface` to `new LoggingExceptionHandler($inner, $logger)` via `instance()`. The general rule is that `boot()` must not re-bind core services; decorating (wrapping, never replacing) the handler is the documented carve-out (root `CLAUDE.md` § Service Providers). It must run in `boot()`, not `register()`, because it needs the resolved inner handler and logger. Ordering caveat: a provider booted *after* this one that re-binds the handler drops the logging wrapper — register such providers before `LogServiceProvider`.
